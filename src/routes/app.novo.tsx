...
import { saveEvent, type Event } from "@/lib/auth";
...
const colorMap: Record<Event["color"], string> = {
  coral: "bg-coral", sun: "bg-sun", mint: "bg-mint", grape: "bg-grape", sky: "bg-sky",
};

function NewEventPage() {
  const navigate = useNavigate();
  const [form, setForm] = useState<Omit<Event, "id">>({
    title: "", emoji: "🎉", date: "", location: "", description: "",
    category: "festa", attendees: 0, capacity: 50, color: "coral",
  });
    ...
    <div className="max-w-2xl mx-auto animate-pop-in">
      ...
      <form onSubmit={submit} className="mt-8 bg-card rounded-3xl p-6 md:p-8 shadow-soft space-y-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Data e hora" type="datetime-local" value={form.date} onChange={(v) => set("date", v)} />
          <div>
            <label className="text-sm font-medium">Categoria</label>
            <select value={form.category} onChange={(e) => set("category", e.target.value as Event["category"])}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Capacidade" type="number" value={String(form.capacity)} onChange={(v) => set("capacity", Number(v) || 0)} />
          <Field label="Confirmados" type="number" value={String(form.attendees)} onChange={(v) => set("attendees", Number(v) || 0)} />
        </div>

        <button type="submit" className="w-full py-4 rounded-2xl bg-primary text-primary-foreground font-semibold text-lg shadow-pop hover:scale-[1.02] transition">
          Criar evento 🎉
        </button>
      </form>
    </div>
